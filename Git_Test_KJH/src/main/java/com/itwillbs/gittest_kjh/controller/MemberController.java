...
import com.itwillbs.gittest_kjh.service.MemberService;
...
//		BCryptPasswordEncoder passwordEncoder = new BCryptPasswordEncoder();
		...
		// 3. 암호화된 패스워드를 다시 MemberVO 객체의 passwd 값에 저장(덮어쓰기)
		...
		// 실패 시 : result_process/fail.jsp 페이지 포워딩("msg" 속성값으로 "회원가입 실패!" 저장)
	...
	//    세션 정보를 관리하는 HttpSesssion, 응답 페이지로 데이터 전송할 Model 타입 파라미터 필요!
	@PostMapping("MemberLogin")
	public String login(MemberVO member, String rememberId, BCryptPasswordEncoder passwordEncoder, HttpSession session, Model model) {
		...
		// => 기존 MemberVO 객체에 조회 결과를 덮어써도 되지만, 기존 패스워드를 유지하기위해 별도의 변수 사용
...
//		}
		...
		if(dbMember == null || !passwordEncoder.matches(member.getPasswd(), dbMember.getPasswd())) { // 로그인 실패
			...
			// - 세션 객체에 저장된 "prevURL" 속성이 null이 아닐 경우 해당 주소로 리다이렉트 하고
	...
	public String memberInfo(MemberVO member, HttpSession session, Model model, HttpServletRequest request) {
		...
		if(id == null) {
			...
			// 로그인 완료 시 해당 주소로 리다이렉트를 수행할 수 있다!
//			session.setAttribute("prevURL", "/MemberInfo");
		...
		// => 파라미터 : MemberVO 객체     리턴타입 : MemberVO (기존 변수 member 재사용)
		// => 이 때, 아이디 파라미터가 없으므로 session 아이디 값을 MemberVO 객체에 저장
		// => 조회 결과 저장 시 새로운 MemberVO 타입 변수 선언 대신 기존 변수 member 재사용 가능
		//    (기존 MemberVO 객체와 별개로 구별해야 할 정보가 없기 때문에 덮어써도 무관함)
		member.setId(id);
		member = memberService.getMember(member);
		
		// Model 객체에 MemberVO 객체 저장 후 member/member_info.jsp 페이지로 포워딩
		model.addAttribute("member", member);
		
		return "member/member_info";
	}
	// ------------------------------------------------------------------------
	// 회원 상세정보 수정("MemberModify" - POST)
	// => 회원 정보 수정 폼 없이 회원 상세정보 페이지에서 바로 수정 페이지 요청함
	@PostMapping("MemberModify")
//	public String memberModify(MemberVO member, String oldPasswd, HttpSession session, Model model) {
	public String memberModify(@RequestParam Map<String, String> map, String[] hobby,
		BCryptPasswordEncoder passwordEncoder, HttpSession session, Model model, HttpServletRequest request) {
		// 만약 파라미터 매핑용 매개변수를 Map 타입으로 선언 시 @RequestParam 어노테이션이 필수!
		// 만약 Map 타입 파라미터와 MemberVO 타입 파라미터를 동시에 선언 시 두 객체 모두 파라미터가 저장됨
		// => 단, Map 객체에는 기존 패스워드(oldPasswd) 파라미터도 저장이 됨(MemberVO 에는 없음)
...
//						session.setAttribute("prevURL", "/MemberInfo");
		...
		// ------------------------------------------------------------------------
		// Map 객체에 세션아이디 덮어쓰기 (키: id)
		map.put("id", id);
		
		// MemberService - getMemberPasswd() 메서드 재사용하여 암호화된 패스워드 조회
		// => 파라미터: 아이디(String)    리턴타입: String(dbPasswd)
		String dbPasswd = memberService.getMemberPasswd(id);
		// 조회된 패스워드와 입력받은 기존 패스워드를 비교(Map 객체의 oldPasswd 키 사용)
		if(dbPasswd == null || !passwordEncoder.matches(map.get("oldPasswd"), dbPasswd)) { // 아이디 또는 패스워드 불일치
			model.addAttribute("msg", "수정 권한이 없습니다!");
			return "result/fail";
		}
		// ------------------------------------------------------------------------
		// 기존 비밀번호 일치 시 회원 정보 수정 전에 새 비밀번호 입력 여부를 판별하여 
...
//		System.out.println("취미: " + Arrays.toString(hobby));
		...
		// => success.jsp 파일과 fail.jsp 파일을 하나로 통합하여 처리해도 무관함
		if (updateCount > 0) {
...
//						session.setAttribute("prevURL", "/MemberInfo");
		...
		// member/member_withdraw_form.jsp 페이지로 포워딩
	...
	public String withdraw(String passwd, BCryptPasswordEncoder passwordEncoder, HttpSession session, Model model) {
		...
		if(dbPasswd == null || !passwordEncoder.matches(passwd, dbPasswd)) { // 아이디 또는 패스워드 불일치
