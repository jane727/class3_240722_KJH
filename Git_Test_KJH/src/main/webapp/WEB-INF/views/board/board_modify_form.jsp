			...
			<table>
				<tr>
					...
					<td class="write_td_right">
						<%-- 글쓴이 작성자는 세션 아이디값을 그대로 사용하므로 그냥 출력 (읽기 전용) --%>
						<input type="text" id="board_name" name="board_name" value="${sessionScope.sId}" readonly required="required" />
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="write_td_right">
						<textarea id="board_content" name="board_content" rows="15" cols="40" required="required">
							${board.board_content}
						</textarea>
					</td>
				</tr>
				<tr>
					...
					<td class="write_td_right">
						<%--
						파일명 존재 여부를 판별하여 업로드 버튼 or 파일명과 삭제버튼 표시
						forEach 태그 활용하여 fileList 객체 요소 갯수만큼 반복
						=> 반복 요소 인덱스 값 또는 순서번호 확인을 위해 varStatus 속성 사용
						=> 반복 인덱스: 속성명.index   반복 순서번호: 속성명.count
						--%>
						<c:forEach var="file" items="${fileList}" varStatus="status">
							<div class="board_file">
								<%-- 파일명 존재할 경우 원본파일명 표시하고 --%>
								<%-- 아니면, 파일 업로드 버튼 표시 --%>
								<c:choose>
									<c:when test="${not empty file}">
										<%-- 원본파일명 출력(varStatus 속성명.index 활용) --%>
										...
										<%-- 파일 다운로드 링크<버튼) 생성 --%>
		 								<a href="${pageContext.request.contextPath}/resources/upload/${file}" download="${originalFileList[status.index]}">
		 									<img src="${pageContext.request.contextPath}/resources/images/download-icon.png" class="img_btn" title="다운로드" alt="다운로드">
		 								</a>
		 								<%-- 파일 삭제 링크(delete-icon.png 이미지) 생성(개별 삭제 용도) --%>
		 								<%-- 하이퍼링크 클릭 시 deleteFile() 함수 호출(파라미터: 글번호, 실제파일명, 카운트번호) --%>
		 								<a href="javascript:deleteFile(${board.board_num}, '${file}', ${status.count})">
		 									<img src="${pageContext.request.contextPath}/resources/images/delete-icon.png" class="img_btn" title="다운로드" alt="다운로드">
		 								</a>
		 								<%-- 만약, 파일명만 표시할 때 파일 업로드 요소도 파라미터로 전달하려면 (파라미터값은 널스트링으로 날아감) --%>
		 								<%-- 파일 업로드 요소 추가 후 hidden 속성으로 숨김 처리 가능 --%>
										<input type="file" name="file${status.count}" hidden>
									</c:when>
									<c:otherwise>
		 								<%-- 
		 								파일 항목의 name 속성값을 다르게 부여하기 위해 file 문자열과 count 속성값 결합 
		 								=> forEach의 status.count 활용 (1부터 1씩 증가됨) => ex) "file" + 1 = "file1"
		 								--%>
