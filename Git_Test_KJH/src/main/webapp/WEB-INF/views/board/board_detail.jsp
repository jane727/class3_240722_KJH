...
<!-- 						<div> -->
							...
							=> download 속성에 속성값 지정 시 지정된 이름으로 다운로드 파일명 변경
							--%>
...
<%-- 							</a> --%>
							...
							<%--    단, _ 기호가 두개 이상일 경우 반복문을 통해 문자열 결합 필요 --%>
...
<%-- 							fileLength : ${fileLength}, delimIndex: ${delimIndex} --%>
							<%-- 2-3) substring() 함수 활용하여 추출(파일명, 구분자 인덱스 활용) --%>
...
<%-- 						</c:forEach> --%>
						...
						<%-- 기존 반복 객체 저장 후 다른 객체는 varStatus 속성을 통해 index 값 사용 가능 --%>
		...
		<section id="commandCell">
			...
			<c:if test="${not empty sessionScope.sId}">
				...
				<%-- 수정, 삭제는 작성자가 세션 아이디와 동일할 경우에만 표시 --%>
				<%-- 단, 관리자의 경우 다른 작성자의 게시물에도 수정, 삭제 버튼 표시 --%>
